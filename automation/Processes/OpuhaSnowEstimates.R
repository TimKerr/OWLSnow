...
#February 2019 Tim Kerr. Updated so that directories match the new server "appserver-01" and the new directory structure that Wayne (IT Manager) has defined.
  ...
  
{
  ...
  if (!require(xml2)) install.packages("xml2",repos= "https://cran.stat.auckland.ac.nz"); library(xml2)    #Package to handle the xml format and structure
  ...
  MergeTo1V2 <- function(ListOfCompleteDataZoo,ListOfNewDataZoos){
  ...
  #CompleteECSData                 <- Map(MergeTo1,CompleteECSData, ListsOfTimeSeries)
  CompleteECSData                 <- MergeTo1V2(CompleteECSData,ListsOfTimeSeries)
  ...
  ClimateData                      <- merge(LakeOpuhaDailyRain,FoxPeakDailyMaxTemperature,FoxPeakDailyMinTemperature)
...
#CompleteDataFileName="\\\\aqualinc-sbs\\data\\ARL Projects\\Other\\C15114_OWL snow storage estimates\\automation\\Input\\OpuhaClimateDataComplete.csv"
{
 ...
 #                           index.column=1,sep=",",format="%y/%m/%d",header=TRUE,regular=FALSE)
  ...
  #Both$Precip<- ifelse(is.na(Both$Precip.RecentData),Both$Precip.PrimaryData,Both$Precip.PrimaryData) #Temporary fix while Fairlie rainfall is in error. Simply default to the primary data, which needs to updated manually each day.
...
GraphOpuhaSnowM3Y<-function(InputFileName = "\\\\aqualinc-sbs\\data\\ARL Projects\\Other\\C15114_OWL snow storage estimates\\automation\\Input\\OpuhaSnowDataComplete.csv",CurrentYear= 2017)
{
  ...
  ColourPalette <- setNames(ColourPalette, colnames(GraphData[,-1]))
  GraphRange <- as.numeric(as.POSIXct(c(paste0(CurrentYear,"-01-01"),paste0(CurrentYear + 1,"-01-02"))))*1000 #This is in milliseconds for some reason
    ...
    py2 <- plot_ly(GraphData, x = as.Date(GraphData$Date), y = ~GraphData$'Percentile 95th', name = 'Percentile 95th',line= list(color = "lightgrey",width=3), mode="lines", type = 'scatter') %>%
    ...
    add_trace(y = ~Current.Year, name = 'Current year', line = list(color = "red",width=3), hoverinfo = 'text', text = ~paste(format(Date, "%e %b"),'<br>', sprintf('%.2f',Current.Year), 'x10<sup>6</sup> m<sup>3</sup>')) %>%
...
#setwd("M:\\Processes")
#setwd("H:\\Other\\C15114_OWL snow storage estimates\\automation\\Processes") #On Tim's home computer
ProcessesDirectory<-getwd()
BaseDirectory=normalizePath(dirname(ProcessesDirectory))
#BaseDirectory="\\\\aqualinc-sbs\\data\\ARL Projects\\RD Projects\\RD18004_Lake Opuha Further Work\\R Processing"
...
#ECSFile = file.path(InputWorkingDirectory,"ToAqualinc.zip")  #for testing
...
if(as.numeric(format(Sys.Date(),"%m")) < 4){CurrentHydrologicalYear <- CurrentHydrologicalYear - 1}
